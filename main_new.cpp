...
vector<int> find_best_path2(double **pher) {
    vector<int> path;
    path.push_back(0);
    int current_node = 0;
    int counter = 0;
    while (current_node != number_of_vertics - 1) {
        ...
        for (int i = 0; i < number_of_vertics; i++) {
            if (i != current_node) {
                pher_sum += pher[current_node][i];
            }
        }
        double r = (double)rand() / RAND_MAX;
        double roulette_wheel_position = 0.0;
        int next_node = -1;
        for (int i = 0; i < number_of_vertics; i++) {
            if (i != current_node) {
                roulette_wheel_position += pher[current_node][i] / pher_sum;
        ...
        if (next_node == -1) {
            ...
            next_node = candidate_nodes[rand() % candidate_nodes.size()];
        }
...
int arboreal_ants(const char *path) {
	...
	if (!has_short && loop){
		cout<<"==========Serching by russian roulette.============\n";
		best_path = find_best_path2(pher);
